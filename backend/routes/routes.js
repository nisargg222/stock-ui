...
router.post('/signin', (request, response) => {
    ...
    signUpTemplateCopy.findOne({ 'username': signedInUserReq.username }, 'password', function (err, signedInUserDb) {
...
router.get('/mystocks', (request, response) => {
    const userStockUserReq = new userStockTemplateCopy({
        username: request.body.username
    })
    userStockTemplateCopy.find({'username': userStockUserReq.username})
    .then(data => {
        response.json(data)
    })
    .catch(error => {
